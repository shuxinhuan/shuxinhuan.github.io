...
function Publication() {
  ...
  const drawConnections = () => {
    ...
    // Draw connections: tag → paper → author
    paperList.forEach((paper) => {
      const paperEl = Array.from(paperElements).find((el) =>
        el.textContent?.includes(paper.title)
      ) as HTMLElement | undefined
      ...
      paper.tags.forEach((tag) => {
        ...
        const color = isPaperOrTagHovered || isTagSelected ? 'rgba(202, 102, 44, 1)' : 'rgba(88, 88, 88, 1)'
      ...
      authors.forEach((author) => {
        ...
        const color = isPaperOrAuthorHovered || isAuthorSelected ? 'rgba(202, 102, 44, 1)' : 'rgba(88, 88, 88, 1)'
  ...
  const reservedAuthors: string[] = ['Benjamin Bach', 'Huamin Qu', 'Yingcai Wu', 'Shixia Liu', 'Di Weng', 'Bongshin Lee', 'Liwenhan Xie', 'Junxiu Tang']
  ...
  return (
    <div id="publication" 
      ...
      <div className="tag-list"
        ...
        {displayTagList.map((tag) =>
          <div key={tag.name}
            ...
            <span style={{
              borderBottom: selectedTags.has(tag.name) ? '2px solid rgba(202, 102, 44, 0.8)' : 'none',
            }}>
              {tag.name}
            </span>
          </div>
      ...
      <div className="paper-list"
        ...
        <span>* denotes equal contribution</span> <br/>
        {(selectedTags.size > 0 || selectedAuthors.size > 0) ? 
          <div style={{
            ...
            alignItems: 'center', 
            marginTop: 10,
            background: 'rgba(202, 102, 44, 0.05)',
            padding: '8px 0px',
            borderRadius: 4,
            fontSize: 14,
          }}>
        ...
        {getPaperList('all').map((paper: PAPER, index: number) =>
          ...
          >
            <Paper
              selected={paper}
              style={{
                background: hoveredPaper === null ? 'transparent' : hoveredPaper === paper.title ? 'rgba(202, 102, 44, 0.05)' : 'transparent',
        ...
        {visibleAuthorList.map((author, idx) =>
          ...
          (<div key={author.name}
            ...
            style={{
              ...
              <span style={{
                borderBottom: selectedAuthors.has(author.name) ? '2px solid rgba(202, 102, 44, 0.8)' : 'none',
              }}>{author.name}</span>
          </div>) : null
        )}
      </div>
